...
async function createBlobFromLocalPath(containerClient, blobName, buffer, blobTags){
  console.log(`creating blob ${blobName} from buffer`);
  const blockBlobClient = await containerClient.getBlockBlobClient(blobName);

  const uploadBlobResponse = await blockBlobClient.uploadData(buffer, { tags: blobTags} );
...
async function main(blobServiceClient){
  ...
  for (let i=0; i<10; i++){
    ...
    blobs.push(createBlobFromLocalPath(containerClient, `${containerName}-${i}.jpg`, buffer, blobTags));
  }
